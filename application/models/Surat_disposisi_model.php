<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Surat_disposisi_model extends CI_Model
{

    public $table = 'surat_disposisi';
    public $id = 'id';
    public $order = 'DESC';

    function __construct()
    {
        parent::__construct();
    }

    // get all
    function get_all()
    {
        $this->db->order_by($this->id, $this->order);
        return $this->db->get($this->table)->result();
    }

    // get data by id
    function get_by_id($id)
    {
        $this->db->where($this->id, $id);
        return $this->db->get($this->table)->row();
    }
    
    // get data by id
    function seek_by_id($id)
    {
        
        return $this->db->query("SELECT * FROM surat_disposisi WHERE id_surat = $id ")->result();
        //$this->db->where($this->id_surat, $id);
        //return $this->db->get($this->table)->result();
    }
    
   
    
    // get total rows
    function total_rows($q = NULL) {
        $this->db->like('id', $q);
	$this->db->or_like('id_surat', $q);
	$this->db->or_like('kpd_yth', $q);
	$this->db->or_like('isi_disposisi', $q);
	$this->db->or_like('sifat', $q);
	$this->db->or_like('batas_waktu', $q);
	$this->db->or_like('catatan', $q);
	$this->db->from($this->table);
        return $this->db->count_all_results();
    }

    // get data with limit and search
    function get_limit_data($limit, $start = 0, $q = NULL) {
        $this->db->order_by($this->id, $this->order);
        $this->db->like('id', $q);
	$this->db->or_like('id_surat', $q);
	$this->db->or_like('kpd_yth', $q);
	$this->db->or_like('isi_disposisi', $q);
	$this->db->or_like('sifat', $q);
	$this->db->or_like('batas_waktu', $q);
	$this->db->or_like('catatan', $q);
	$this->db->limit($limit, $start);
        return $this->db->get($this->table)->result();
    }

    // insert data
    function insert($data)
    {
        $this->db->insert($this->table, $data);
    }

    // update data
    function update($id, $data)
    {
        $this->db->where($this->id, $id);
        $this->db->update($this->table, $data);
    }

    // delete data
    function delete($id)
    {
        $this->db->where($this->id, $id);
        $this->db->delete($this->table);
    }

}

/* End of file Surat_disposisi_model.php */
/* Location: ./application/models/Surat_disposisi_model.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2016-05-01 17:53:25 */
/* http://harviacode.com */